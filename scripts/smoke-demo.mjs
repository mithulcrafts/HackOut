...
assert.match(manifest, /VidyutSutra/);
const serviceWorker = await call("/sw.js");
assert.match(serviceWorker, /vidyutsutra-shell/);
...
await call(`/api/activities/${created.activity.id}/lifecycle`, { action: "pause" });
detail = await call(`/api/activities/${created.activity.id}`);
assert.equal(detail.activity.status, "paused");
await call(`/api/activities/${created.activity.id}/lifecycle`, { action: "resume" });
detail = await call(`/api/activities/${created.activity.id}`);
assert.equal(detail.activity.status, "recommended");
await call(`/api/activities/${created.activity.id}/lifecycle`, { action: "remove" });
await call(`/api/activities/${created.activity.id}`, undefined, 404);
...
await reset();
const custom = await call("/api/activities", { type: "Custom", name: "Laundry", earliestStart: "10:00", latestFinish: "17:00", durationHours: 1, powerKW: 1.5, interruptible: false }, 201);
assert.equal(custom.activity.type, "Custom");
assert.ok(Array.isArray(custom.schedulePreview));
assert.ok(custom.offer, "A custom load should receive an offer in the seeded programme");
state = await call(`/api/consumer?offerId=${custom.offer.id}`);
state = await action(state, "accept");
state = await action(state, "simulate", { outcome: "success" });
state = await action(state, "verify");
assert.equal(state.verification.status, "verified");
...
assert.equal(savedProfile.profile.programme_name, "Smoke programme");
await reset();
