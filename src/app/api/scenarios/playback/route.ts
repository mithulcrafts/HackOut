...
import { advanceSimulatedOffer, recordSimulatedOffer, verifyScenarioOffer } from "@/domain/playback";
import { requireOperatorAccess } from "@/lib/operator-access";

const playbackSchema = z.object({
  activityId: z.string().min(1).max(120),
  outcome: z.enum(["success", "partial", "late", "missing", "rebound"]).default("success"),
  action: z.enum(["full", "advance"]).default("full"),
}).strict();
...
export async function POST(request: Request) {
  ...
  try {
    const recorded = parsed.data.action === "advance"
      ? advanceSimulatedOffer(scenario, offer.id, parsed.data.outcome)
      : recordSimulatedOffer(scenario, offer.id, parsed.data.outcome);
    ...
    const response = NextResponse.json({
      ...
      playbackAction: parsed.data.action,
      playbackComplete: verification.outcome !== "pending",
