...
export async function POST(request: Request) { const access = await requireOperatorAccess(request); if (access.mode === "error") return access.response; const session = access.session; const response = NextResponse.json({ scenario: resetScenario(session), message: "Demo scenario reset.", data_source: "simulation" }); if (access.mode === "demo") setDemoCookie(response, session); return response; }
