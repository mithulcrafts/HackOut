...
function Plot({ data, series, unit, bar = false }: { data: SeriesRow[]; series: { key: string; name: string; color: string }[]; unit: string; bar?: boolean }) {
  ...
  return <><p className="chart-unit">{unit} · times in IST · forecast and meter data</p><div className="consumer-chart"><ResponsiveContainer width="100%" height="100%">{bar ? <BarChart data={data}>{children}{series.map(s => <Bar key={s.key} dataKey={s.key} name={s.name} fill={s.color} isAnimationActive={false} />)}</BarChart> : <LineChart data={data}>{children}{series.map(s => <Line key={s.key} type="stepAfter" dataKey={s.key} name={s.name} stroke={s.color} dot={false} strokeWidth={2} isAnimationActive={false} />)}</LineChart>}</ResponsiveContainer></div><details className="evidence-readings"><summary>View chart data</summary><div className="evidence-scroll" role="region" aria-label="Chart data" tabIndex={0}><table><caption>{unit} · forecast and meter data</caption><thead><tr><th scope="col">Time (IST)</th>{series.map(s => <th scope="col" key={s.key}>{s.name} ({unit})</th>)}</tr></thead><tbody>{data.map((row, i) => <tr key={i}><td>{row.time}</td>{series.map(s => <td key={s.key}>{row[s.key]}</td>)}</tr>)}</tbody></table></div></details></>;
}
