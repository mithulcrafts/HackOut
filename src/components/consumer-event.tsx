...
export function ConsumerEvent({ inbox = false }: { inbox?: boolean }) {
  ...
  return <>
    ...
    <p className="form-message" role="status" aria-live="polite">{busy ? "Savingâ€¦" : message}</p>
    {inbox && choices.length > 0 && <label className="filter-control">Choose an offer<select value={o?.id ?? selectedOfferId} onChange={e => selectOffer(e.target.value)}><option value="">All offers</option>{choices.map(choice => <option key={choice.id} value={choice.id}>{choice.name} Â· {choice.decision}</option>)}</select></label>}
    ...
    {o && state && (filter === "all" || filter === o.decision) && <article className="schedule-card event-card">
      <span className="status-pill absorb">{o.decision} Â· {presentation?.objective === "protect" ? "Protect peak" : "Absorb renewable event"}</span><div className="section-heading"><h2>{o.name}</h2><ListenButton text={`${o.name}. Move an existing task into a renewable-aligned window. Ready by ${slotTime(o.deadline_slot)}.`} /></div>
      <p>Move an existing task into a renewable-aligned window. You decide whether the time works.</p>
      <dl className="detail-grid"><div><dt>Usual time</dt><dd>{slotTime(o.baseline_start)}â€“{slotTime(o.baseline_start + o.duration_slots)}</dd></div><div><dt>{o.decision === "accepted" ? "Accepted" : "Proposed"} time</dt><dd>{slotTime(o.proposed_start)}â€“{slotTime(o.proposed_start + o.duration_slots)}</dd></div><div><dt>Ready by</dt><dd>{slotTime(o.deadline_slot)} IST</dd></div><div><dt>Task requirement</dt><dd>{o.required_kwh} kWh Â· {o.power_kw} kW limit</dd></div></dl>
      <div className="reward-preview"><strong>Up to {presentation?.estimatedPoints ?? 0} points</strong><span>â‚¹{(presentation?.estimatedRupees ?? 0).toFixed(2)} illustrative reward Â· only for verified eligible energy</span></div>
      <p>Rate: â‚¹{presentation?.rewardRate ?? 1.5}/kWh, capped at â‚¹{presentation?.rewardCap ?? 12} per event. No live funding or payment.</p>
    ...
    {state && <article className="schedule-card"><div className="section-heading"><h2>Notifications</h2><ListenButton text={state.notifications.map(n=>n.message).join(". ")} /><button className="secondary-button" disabled={busy || !state.notifications.some(n => !n.read_at)} onClick={markRead}>Mark all read</button></div>{state.notifications.length ? state.notifications.map(n => <div className="notification-item" key={n.id}><p>{!n.read_at && <strong>New Â· </strong>}{n.message}</p><small>{new Date(n.created_at).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })} IST</small></div>) : <p>No notifications yet.</p>}</article>}
  </>;
}
