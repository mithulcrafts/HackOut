...
export function ConsumerEvent({ inbox = false }: { inbox?: boolean }) {
  ...
  return <>
    ...
    {o && state && (filter === "all" || filter === o.decision) && <article className="schedule-card event-card">
      ...
      {o.simulation_run && (!state.verification || state.verification.status === "pending") && <div className="notice"><p>{state.verification?.status === "pending" ? "Verification is waiting for meter or device readings." : `${state.readings.length} simulated meter readings are ready.`}</p><button className="primary-button" disabled={busy} onClick={() => action("verify")}>Verify readings</button></div>}
