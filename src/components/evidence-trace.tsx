...
export function EvidenceTrace({ rows }: { rows: EvidenceTraceRow[] }) {
  ...
  return <section className="evidence-trace" aria-label="Uploaded cumulative energy trace">
    ...
    <div className="consumer-chart"><ResponsiveContainer width="100%" height="100%"><LineChart data={rows}><CartesianGrid stroke="var(--line)" strokeDasharray="3 3" /><XAxis dataKey="time" minTickGap={28} tick={{ fill: "var(--muted)", fontSize: 10 }} /><YAxis unit=" kWh" tick={{ fill: "var(--muted)", fontSize: 10 }} /><Tooltip contentStyle={{ background: "var(--surface)", borderColor: "var(--line)", borderRadius: 12 }} /><Line type="stepAfter" dataKey="cumulative" name="Cumulative energy" stroke="var(--blue)" dot={false} strokeWidth={2} isAnimationActive={false} /></LineChart></ResponsiveContainer></div>
