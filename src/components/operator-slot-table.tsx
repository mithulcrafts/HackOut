...
export function OperatorSlotTable({ forecast, balances }: { forecast: ForecastSlot[]; balances: BalanceResult[] }) {
  const indexes = [12, 20, 26, 34, 42].filter((index) => Boolean(forecast[index] && balances[index]));
  return <Table><TableCaption className="label" style={{ textAlign: "left" }}>Selected operating points · slot-average kW</TableCaption><TableHeader><TableRow><TableHead>Time</TableHead><TableHead>Renewable</TableHead><TableHead>Demand</TableHead><TableHead>Mode</TableHead></TableRow></TableHeader><TableBody>{indexes.map((index) => <TableRow key={index}><TableCell>{formatSlot(index)}</TableCell><TableCell>{forecast[index].renewableKW.toFixed(2)} kW</TableCell><TableCell>{(forecast[index].renewableKW - balances[index].balanceKW).toFixed(2)} kW</TableCell><TableCell>{balances[index].mode === "absorb" ? "Absorb" : "Protect"}</TableCell></TableRow>)}</TableBody></Table>;
}
