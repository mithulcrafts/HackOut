...
export function SimulationConsole({ activities, forecast }: { activities: Activity[]; forecast: ForecastSlot[] }) {
  ...
  return <main className="shell">
    ...
    <div className="container" style={{ paddingTop: 40 }}>
      <div className="eyebrow">Operations review</div><h1>Review the demand-response day.</h1>
      <p className="muted">Compare estimated supply and demand, then review the recorded evidence for an accepted activity. No device or grid command is sent.</p>
      {(busy || message) && <p role={isError ? "alert" : "status"} aria-live="polite" className="muted">{busy ? "Updating the operating view…" : message}</p>}
      <div className="grid two-col">
        <section className="card"><h2>What-if preview</h2>
          <label className="label" htmlFor="source">Forecast input</label>
          <select id="source" value={source} onChange={event => setSource(event.target.value as "simulation" | "weather")} disabled={busy} style={{ width: "100%", margin: "8px 0 16px" }}><option value="simulation">Planning baseline</option><option value="weather">Open-Meteo weather estimate</option></select>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}><button className="status min-h-11" onClick={runPreview} disabled={busy}>Calculate outlook</button><button className="source min-h-11" onClick={reset} disabled={busy}>Restore baseline</button></div>
          {preview?.metadata && <p className="muted" style={{ fontSize: ".8rem" }}>{preview.metadata.location ? `Weather estimate for ${preview.metadata.location} · ${preview.metadata.weatherDate}` : "Planning baseline"}{preview.metadata.fallback ? " · fallback to planning baseline" : ""}</p>}
          <div className="chart-wrap" style={{ marginTop: 16 }}><div className="chart" style={{ minWidth: 480, height: 170 }} aria-label="Estimated renewable and fixed demand by half-hour slot in kW">{visibleForecast.map((slot) => <div className="bar-group" key={slot.index}><div className="bar renewable" style={{ height: `${Math.min(100, slot.renewableKW * 7)}%` }} title={`${slot.start}: renewable ${slot.renewableKW.toFixed(2)} kW`} /><div className="bar demand" style={{ height: `${Math.min(100, slot.fixedDemandKW * 7)}%` }} title={`${slot.start}: fixed demand ${slot.fixedDemandKW.toFixed(2)} kW`} /></div>)}</div></div>
          <p className="muted" style={{ fontSize: ".8rem" }}>Renewable supply and fixed demand · slot-average kW · 00:00–24:00 IST · {preview?.data_source ?? "planning baseline"}</p>
        ...
        <section className="card"><h2>Evidence review</h2><p className="muted">Only an accepted activity can run. A recorded meter reading is checked against the accepted window, original baseline, energy requirement and deadline.</p>
          {!activities.some((activity) => activity.status === "accepted") && <p className="muted">No accepted activity is ready. <Link href="/consumer/offers" className="text-link">Open consumer offers</Link> to accept one, to review a different outcome.</p>}
          <div className="grid" style={{ gap: 10 }}>{activities.map((activity) => <div key={activity.id} style={{ background: "var(--surface-2)", borderRadius: 10, padding: 12 }}><strong>{activity.name}</strong><div className="muted" style={{ fontSize: ".8rem", margin: "4px 0 10px" }}>{activity.requiredEnergyKWh} kWh · {activity.durationSlots * 30} minutes · {activity.status}</div><label className="label" htmlFor={`outcome-${activity.id}`}>Outcome</label><select id={`outcome-${activity.id}`} value={selectedOutcomes[activity.id] ?? "success"} onChange={(event) => setSelectedOutcomes((current) => ({ ...current, [activity.id]: event.target.value as PlaybackOutcome }))} disabled={busy || activity.status !== "accepted"} style={{ width: "100%", minHeight: 44, margin: "8px 0", padding: 8, color: "var(--text)", background: "var(--surface)", border: "1px solid var(--line)", borderRadius: 8 }}>{outcomes.map((outcome) => <option key={outcome.value} value={outcome.value}>{outcome.label}</option>)}</select><button className="source min-h-11" onClick={() => playback(activity.id)} disabled={busy || activity.status !== "accepted"}>Record and verify</button>{activity.status !== "accepted" && <p className="muted" style={{ fontSize: ".8rem", marginBottom: 0 }}>{activity.status === "verified" || activity.status === "failed" ? "This scenario is complete. Review the result above." : "Waiting for the consumer to accept a schedule."}</p>}</div>)}</div>
          {verification && <div className="card" style={{ marginTop: 16 }}><div className="eyebrow">Verification result · {verification.outcome}</div><p>{verification.reason}</p><p className="muted">Required: {verification.requiredEnergyKWh} kWh · Recorded: {verification.recordedEnergyKWh} kWh · Eligible shift: {verification.eligibleShiftedKWh}</p><Link className="text-link" href="/operator/overview">View updated operator response →</Link></div>}
        </section>
      </div>
      <p className="footer">Missing readings remain pending. Incomplete, late or added consumption does not count as a verified response. Supply and demand values are estimates; this view does not operate real equipment or transfer money.</p>
    </div>
  </main>;
}
