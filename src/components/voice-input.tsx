...
export function VoiceInput({ onText }: { onText: (text: string) => void }) {
  function start() {
    ...
    if (!Speech) return;
    const recognition = new Speech(); recognition.lang = window.localStorage.getItem("vidyut_language") || "en-IN"; recognition.onresult = event => onText(event.results[0][0].transcript); recognition.start();
  }
