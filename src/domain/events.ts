...
export function closeEvent(scenario: Scenario, eventId: string): Scenario {
  if (!scenario.events.some((event) => event.id === eventId)) throw new Error("Event not found.");
  return { ...scenario, events: scenario.events.map((event) => event.id === eventId ? { ...event, status: "closed" as const } : event) };
}
...
export function decideOffer(scenario: Scenario, offerId: string, decision: "accept" | "skip" | "override", proposedStart?: number): Scenario {
