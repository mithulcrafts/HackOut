...
export function consumerView(state: ConsumerState, outlook: typeof outlookFixture | Scenario = outlookFixture, outlookSource = "Planning estimate", terms?: Terms) {
  ...
  return {
    ...
    presentation: {
      ...
      rewardEligible: state.offer?.reward_eligible !== false,
      estimatedPoints: state.offer && state.offer.reward_eligible !== false ? Math.floor(Math.min(state.offer.required_kwh, rewardRate > 0 ? rewardCap / rewardRate : state.offer.required_kwh) * 15) : 0,
      estimatedRupees: state.offer && state.offer.reward_eligible !== false ? Math.min(rewardCap, state.offer.required_kwh * rewardRate) : 0,
