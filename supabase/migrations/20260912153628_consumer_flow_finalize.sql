do $$ begin
 if to_regclass('public.consumer_notifications') is null then
  create table public.consumer_notifications (id uuid primary key default gen_random_uuid(), owner_id uuid not null references auth.users(id) on delete cascade, offer_id uuid not null references public.offers(id) on delete cascade, message text not null, created_at timestamptz not null default now());
  alter table public.consumer_notifications enable row level security;
 end if;
end $$;
-- Recreate policies safely after a partially applied migration.
drop policy if exists own_offers on public.offers;
drop policy if exists own_readings on public.meter_readings;
drop policy if exists own_verifications on public.verifications;
drop policy if exists own_rewards on public.reward_ledger;
drop policy if exists own_notifications on public.consumer_notifications;
