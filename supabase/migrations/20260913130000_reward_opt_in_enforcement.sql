-- Respect a participant's reward preference in the authenticated consumer flow.
--
-- Scheduling and verification remain available when reward_program_opt_in is
-- false.  We simply do not create a reward-ledger entry (and therefore do not
-- expose a pending/verified reward for that activity).  Missing profile rows
-- retain the historical safe default of opting in so existing accounts do not
-- lose settlement unexpectedly.
--
-- This replaces the bounded RPC introduced by the consumer flow migrations;
-- keep its validation and simulation rules unchanged.
...
declare u uuid:=auth.uid(); o public.offers; aid uuid; vid uuid; k integer; run_start integer; energy numeric:=0; step_energy numeric; accepted_energy numeric:=0; baseline_energy numeric:=0; total_energy numeric:=0; eligible numeric:=0; result text; why text; rewards_enabled boolean:=true;
begin
 ...
 if command='reset' then
  ...
  -- Read the preference inside the transaction, after verification is frozen.
  -- A missing profile preserves the historical default (true).
  rewards_enabled := coalesce((select reward_program_opt_in from public.profiles where id=u), true);
  if result='verified' and rewards_enabled then
  ...
  insert into public.consumer_notifications(owner_id,offer_id,message) values(u,o.id,case when result='verified' and not rewards_enabled then 'Verification: verified. Your activity met the accepted window, but reward participation is turned off in your profile; no reward was issued.' else 'Verification: '||result||'. '||why end);
